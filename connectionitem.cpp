...
ConnectionItem::ConnectionItem(NodeScene &scene, const Connection &connection)
    : mScene(scene),
      mConnection(connection)
{
    ...
    path.append({ 24, 24 });
    path.append({ 24 * 3, 24 });
    path.append({ 24 * 3, 24 * 3 });
    path.append({ 24 * 6, 24 * 3 });
    path.append({ 24 * 6, 24 * 7 });
    makeCurve(path);
#else
    updatePath();
#endif
...
void ConnectionItem::updatePath()
{
    ...
    auto c1 = item1->mapToScene(item1->portRect(item1->boundingRect(), Direction::Output, mConnection.port1).center());

    auto item2 = mScene.nodeItem(mConnection.node2);
    auto c2 = item1->mapToScene(item2->portRect(item2->boundingRect(), Direction::Output, mConnection.port2).center());
